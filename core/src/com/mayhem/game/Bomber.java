...
public class Bomber extends ApplicationAdapter implements InputProcessor {
    ...
    public boolean keyDown(int keycode) {
        ...
        if(keycode == Keys.LEFT) {
        	...
        	cell = collisionLayer.getCell(((int)(posX-moveAmount))/32,(int)posY/32);
            if(cell != null) {
            	if(!cell.getTile().getProperties().containsKey("blocked")) {
	            	posX-=moveAmount;
	            	camera.translate(-moveAmount, 0);
	            	camera.update();
	            }
            }
        ...
        if(keycode == Keys.DOWN) {
        	cell = collisionLayer.getCell(((int)posX)/32,(int)(posY-moveAmount)/32);
            if(cell != null) {
            	if(!cell.getTile().getProperties().containsKey("blocked")) {
	            	posY-=moveAmount;            
	            	camera.translate(0, -moveAmount);
	            	camera.update();
	            }
            }
        ...
        if(keycode == Keys.SPACE) {
        	//JOptionPane.showMessageDialog(null, ((int)posX)/32+"+"+(992-(int)posY)/32);
        }
        return true;
    }
